	...
	if (GetVertPos(startVert) != -1) {
		...
		for (int i = 0; i < vertList.size(); i++) {
			if (vertList[i] != startVert) {
				cout << "кратчайшее расстояние от" << startVert << " до" << vertList[i] << "=" << labelList[i] << endl;
			}
		}
		return 0;
	}
...
Graph<char> makeGraph()
{
	Graph<char> graph;
	char amountVerts, amountEdges, sourceVertex, targetVertex, edgeWeight; 
	...
	for (int i = 0; i < amountEdges; ++i) {
		...
		char* sourceVertPtr = &sourceVertex; 
		cout << "Конечная вершина: "; cin >> targetVertex; cout << endl; 
		char* targetVertPtr = &targetVertex; 

		cout << "Вес ребра: "; cin >> edgeWeight; cout << endl;
		graph.addEdge(*sourceVertPtr, *targetVertPtr, edgeWeight); 
	}
	cout << endl;
	graph.Print();
	return graph;
